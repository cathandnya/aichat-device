...
test("**続けて呼ばれたぶんは全部落とす**", () => {
  // 読み上げ中に割り込むときは強めに重ねて呼ぶ。1 個しか落とさないと
  // 残った方が質問として AI に渡り、**同じ答えを繰り返す**（実機で発生）。
  assert.equal(stripWake("ずんだもんずんだもん。", WAKE), "");
  assert.equal(stripWake("ずんだもんずんだもん、止めて。", WAKE), "止めて。");
  assert.equal(stripWake("ずんだもん ずんだもん ずんだもん", WAKE), "");
  // 違う語が並んでも同じ。
  assert.equal(stripWake("ずんだもんすんだもん、今何時。", WAKE), "今何時。");
});
