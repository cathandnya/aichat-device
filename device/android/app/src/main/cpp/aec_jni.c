...
static void suppress(Aec *aec, const float *mic, const float *ref, float *out) {
    ...
    for (int k = 0; k < FFT_HALF; k++) {
        ...
        /*
         * どれだけ引くか。**2.0 倍で足りる。強めても得が無い。**
         *
         * 最大音量で端末に「ずんだもん」を鳴らさせて測った結果:
         *
         *   2.0倍・床8%  ERLE 9〜11dB  自己起動 0 回  聞こえた文字「ランダム。」
         *   3.0倍・床2%  ERLE 9〜15dB  自己起動 0 回  聞こえた文字「あなたの」
         *
         * **どちらも文字にはなるが起動語には化けない。** 3.0 倍にしても
         * 自己起動の防止に足しにならず、**人の声まで消えて割り込みが
         * できなくなる**（`ohr` が 0 segments を返した）だけだった。
         *
         * 消し残りが文字になること自体は許容する。下流はウェイクワードの
         * 一致判定なので、言葉になっても起動語でなければ実害が無い。
         */
        float keep = mag - 2.0f * est;
        float floor_ = 0.08f * mag;
