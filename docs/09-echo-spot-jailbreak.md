...
**マイクは既知の不具合に入っていない。** 動く見込み（[06](06-device-implementation.md) の関門）。
...
1. **マイクが録れるか。** 録音アプリで録るだけ。**ここが駄目なら計画ごと止まる**
   （そのときは自作案に戻る。[02](02-device-selection.md) と
   [06](06-device-implementation.md) に残してある）
2. **エコーキャンセルが有効になるか。** `adb logcat -s aichat` に
   「エコーキャンセル: true/false」が出る。**true なら読み上げ中の割り込みに進める**
3. 顔と縁の輪が 480×480 の丸に収まるか
