...
**話しかける経路は `/ws` の1つだけ。** `/api/stt` `/api/chat` `/api/tts` は
サーバーに口として残っているが、画面からは呼ばない。実機にボタンは無いので、
ボタンで話す経路を持つと実機で困ることに気づけない。
...
**線形の引き算だけでは足りない。** speexdsp（および WebRTC AEC3）は
...
そこで **2 段にしてある**（`aec_jni.c`）。

1. **speexdsp で線形に引ける分を引く**（3〜6dB）。残差抑圧も掛ける
2. **そのうえで 512 点 FFT で、帯域ごとに予想されるエコーのぶんだけ下げる**

2 段目は位相を使わないので歪みに強い。比は走らせながら憶える（**鳴っている
...
**文末でだけ切る。** 読点で切ると「〜親子丼のような丼ものにするか、」のように
**文の途中で読み上げが途切れて不自然**になる。非常口として 200 字の上限を
置いてあるが、これは句点を打たない回答が来たときだけ働く。
...
| 層 | 選定 | 理由 |
|---|---|---|
| ローカルサーバー | **Node 22 + Hono** | Web 標準 API（fetch / Response / ReadableStream / crypto.subtle）だけで書ける。Node 22 は `.ts` をそのまま実行できるのでビルド手順が要らない |
...
| サーバー | Node 22 + Hono **1つだけ**。鍵を持ち、AI を直接呼ぶ |
| 確認用の画面 | ブラウザ（Vite + 素の TypeScript）。手元で内容を読む・試すためのもの |
...
| ウェイクワード | **連続 ASR + 文字列一致**（既定「ずんだもん」）。専用モデルの学習は要らない |
