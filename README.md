...
以前は「ボタンを押して話す」HTTP の経路（`/api/stt` → `/api/chat` → `/api/tts`）も
あり、状態も文の切り出しも読み上げもブラウザ側に二重にあった。**消した。**
実機にボタンは無いので、残しておくと実機で困ることに気づけない。
...
house_power が落ちていても会話は落とさない。「分かりません」と言う。
